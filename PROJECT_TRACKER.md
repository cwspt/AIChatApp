...
- Validation: the focused keep-screen-on/settings tests and full `:app:testDebugUnitTest` suite passed, and `:app:assembleDebug` completed. After reinstalling on `127.0.0.1:5555`, Settings showed the new switch enabled by default; `dumpsys window` showed `KEEP_SCREEN_ON` and the app as `mHoldScreenWindow` during a real generation, then showed neither immediately after generation was stopped.
- Done: active single-chat and group-chat generations now hold a bounded CPU wake lock through the existing foreground service, so natural screen-off does not suspend long reasoning or streaming reads. The AI HTTP client also sends a 30-second HTTP/2 ping during silent reasoning gaps, and generation teardown now waits for both single and group jobs before stopping background protection.
- Done: generation now also holds a Wi-Fi performance lock while active. Explicit socket transport failures such as `Broken pipe`, connection reset, socket close, and unexpected EOF automatically retry once only when no answer text has been emitted; partial answers are never restarted automatically, avoiding duplicated text and unnecessary duplicate requests.
- Done: Settings now exposes the device's battery-optimization status and a direct "allow background generation" system action; single-chat and group-chat transport-error bubbles expose the same action. OpenAI Responses retries reuse a stable idempotency key so supporting providers can associate a reconnect with the original logical request without enabling server-side response storage.
- Risk: direct `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` is appropriate for this personally distributed build's user-requested long-running generation, but Android lint flags it for Play policy review. A future Play-distributed variant should replace the direct confirmation with the general battery-optimization settings page unless its declared core functionality satisfies Play's exemption policy.
- Done: low-level socket abort/reset errors now show a localized streaming-connection interruption message with actionable battery, Wi-Fi, VPN, and proxy checks instead of exposing `Software caused connection abort` directly.
- Validation: all `:app:testDebugUnitTest` tests and `:app:assembleDebug` passed. The Robolectric service lifecycle tests confirm both generation locks are released correctly and the battery-optimization request targets the current package; provider adapter tests cover the zero read timeout, 30-second ping, silent transport retry, no-retry-after-text rule, and stable Responses idempotency key. On `127.0.0.1:5555`, the system background-run confirmation opened from the new Settings action, `deviceidle whitelist` then contained `com.personal.aichat.debug`, and the UI changed to "allowed". `dumpsys power` and `dumpsys wifi` also showed both generation locks while active and neither after stop. `lintDebug` remains blocked by 11 pre-existing project errors and reports one intentional new Play-policy warning for the direct battery-optimization exemption request.
...
- Done: floating range-selection pills now use the same shorter `选择到此` label, keeping list overlays consistent and less obtrusive on narrow screens.
...
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the conversation component split.
...
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the provider settings component split.
- Done: UI splitting continued by extracting provider management list rows and the provider rebind-before-delete dialog into `ProviderManagerComponents.kt`.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the provider manager component split.
- Done: UI splitting continued by extracting settings subcomponents, attachment limit controls, provider config import/QR dialogs, background preset rows/import/editor helpers, and theme preview colors into `SettingsComponents.kt`.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the settings component split.
- Done: UI splitting continued by extracting bot identity labels, bubble palettes/colors, avatar rendering, and shared color helpers into `BotIdentityComponents.kt`.
- Validation: `:app:testDebugUnitTest --tests com.personal.aichat.ChatRepositoryForkTest.botIdentityLabelsAreCompactAndStable` and `:app:assembleDebug` passed after the bot identity component split.
- Done: UI splitting continued by extracting the provider capability matrix, compact badges, and provider capability helpers into `ProviderCapabilityComponents.kt`.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the provider capability component split.
- Done: UI splitting continued by extracting the chat composer, attachment picker menu, retry/stop/send controls, and image-generation option controls into `ComposerComponents.kt`.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the composer component split.
- Done: UI splitting continued by extracting attachment opening, in-app image/PDF/text previews, generated-image grids, and attachment strips into `AttachmentComponents.kt`.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the attachment component split.
- Done: UI splitting continued by extracting the single-chat top bar, conversation overflow/share menus, top metadata strip, group metadata helper, and context-capacity summary text into `TopBarComponents.kt`.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the top bar component split.
- Done: UI splitting continued by extracting single-chat and group-chat tool-call card rendering, expansion controls, share/favorite actions, and tool detail sections into `ToolCallCards.kt`; shared streaming bubble helpers and bot bubble colors are now package-visible for component reuse.
- Validation: `:app:compileDebugKotlin` and `:app:assembleDebug` passed after the tool-call card component split.
...
- Validation: `:app:testDebugUnitTest --tests com.personal.aichat.ChatRepositoryForkTest.toolCallVisualKindVariesByToolName --tests com.personal.aichat.ChatRepositoryForkTest.toolCallDetailsExtractsSearchQueryAndCitations --tests com.personal.aichat.ChatRepositoryForkTest.toolCallDetailsSkipsOpenedUrlAsCitation` and `:app:assembleDebug` passed after the tool-call model split.
- Done: UI splitting has started by moving tested pure list/navigation helpers, grouped tool list models, collapsed group-message summaries, and fork-source label logic out of `AIChatAppRoot.kt` into `UiListModels.kt`.
- Validation: `:app:testDebugUnitTest --tests com.personal.aichat.ChatRepositoryForkTest.conversationForkSourceLabelShowsSourceTitleOrMissingSource --tests com.personal.aichat.ChatRepositoryForkTest.groupToolMessagesStayBeforeMatchingBotMessage --tests com.personal.aichat.ChatRepositoryForkTest.chatToolMessagesGroupBeforeAssistantMessage --tests com.personal.aichat.ChatRepositoryForkTest.longBubbleNavTargetShowsOnlyMissingEdges --tests com.personal.aichat.ChatRepositoryForkTest.collapsedGroupMessageSummaryUsesFirstNonBlankLineAndLimit`, `:app:assembleDebug`, and the pre-commit mojibake scan passed after the first UI helper split.
